...
この Gem の挙動を試すには、`bin/console` を実行します。`jp_prefecture` を読み込んだ状態で irb が起動します。
...
```
irb(main):001> JpPrefecture::Prefecture.find(13).name
=> "東京都"
```

Active Record を試す場合は、`gemfiles/` 配下の Gemfile を指定して、コンソール内で `active_record` を読み込みます。

```bash
BUNDLE_GEMFILE=gemfiles/rails-8-1.gemfile bin/console
```
...
ローカル環境ですべてのバージョンをテストするのは難しいため、最新バージョンの Ruby/Rails でテストがパスすることを最低限確認してください。
...
特定バージョンの Rails でテストを実行するには、以下のようにします。
...
RuboCop でコーディングスタイルをチェックします。自動修正できない警告は手動で修正します。
...
Gem の公開は GitHub Actions の `Release` ワークフローが行います。
...
1. [Actions](https://github.com/chocoby/jp_prefecture/actions) で `release` environment の承認を行う
   - 承認前にタグとバージョンの検証、RuboCop、RSpec が実行される
   - 承認すると Gem が RubyGems.org に公開され、その後リリースノートが `CHANGELOG.md` の内容で自動的に埋まる

## 都道府県の郵便番号データの更新

GitHub Actions の `Update zip code data` ワークフローが毎月 1 日に自動更新します。
手動で実行する場合は以下のタスクを実行します。変更がある場合は `data/zip.yml` に差分が発生します。
